...
export default function ItemDetailPage() {
  ...
  return (
    <PageLayout>
      {/* ヘッダー */}
      <PageHeader
        title={t("dashboard.items.detail.page_title")} 
        description={t("dashboard.items.detail.page_description")}
      />
        ...
        <div className="md:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <h2 className="text-sm font-medium text-vrclo1-500 ">{t("core.data.item.category")}</h2>
                <p className="text-base mt-1 capitalize">{item.category}</p>
              </div>

              <div>
                <h2 className="text-sm font-medium text-vrclo1-500 ">{t("core.data.item.store_url")}</h2>
                {item.storeUrl ? (
                  <a 
                    href={item.storeUrl} 
                    target="_blank" 
          ...
          <Card className="border-dashed">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-vrclo1-500">
                ...
                {t("dashboard.items.detail.used_in_coordinations")}
              </CardTitle>
            </CardHeader>
            <CardContent>
